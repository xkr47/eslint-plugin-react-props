...
module.exports = (context) => {
  ...
  function assertExpectedReactPropSyntax(key, declaration, allowIsRequired) {
    if (declaration.type === 'MemberExpression' &&
      ...
      if (!allowIsRequired) {
        return context.report(declaration, key + ": please move isRequired qualifier outside the oneOfType() declaration e.g. '(React.)PropTypes.oneOfType(...).isRequired'");
      }
      declaration = declaration.object;
    }
    if (declaration.type === 'CallExpression') {
      if (!isPropTypesExpression(declaration.callee) || !isIdentifier(declaration.callee.property)) {
        return context.report(declaration, key + ": unsupported proptypes call syntax");
      }
      var propType = declaration.callee.property.name;
      var propValue = getPropValue(propType);
      switch (propValue) {
        case 3: case 4: case 5: case 6: case 7: break;
        default: return context.report(declaration, key + ': ' + propType + ' must be called like a function');
      }
      if (declaration.arguments.length !== 1) {
        context.report(declaration, key + ': ' + propType + ' expects exactly one argument of type ' + argType[propValue]);
      }
      ...
      if (((propValue == 3 || propValue == 4) && isArray(arg) || isObject(arg))
        ...
        return context.report(declaration, key + ': ' + propType + ' expects exactly one argument of type ' + argType[propValue]);
      }
      switch (propValue) {
        case 3:
        ...
        case 4:
          if (arg.type === 'Literal') {
            context.report(declaration, key + ': ' + propType + ' argument must not be a literal');
          }
          break;
        case 5:
          arg.elements.every(function (elem) {
            if (elem.type !== 'Literal') {
              context.report(declaration, key + ': ' + propType + ' array entries must be literals');
              return false;
            }
            return true;
          });
        ...
        case 7:
          arg.properties.every(function (elem) {
            if (elem.type !== 'Property') {
              context.report(declaration, key + ': ' + propType + ' object must only consist of properties');
              return false;
            }
            if (!isIdentifierOrRawLiteral(elem.key)) {
              context.report(declaration, key + ': ' + propType + ' properties must be identifiers or literals');
            }
      ...
      switch (propValue) {
        case 1: return;
        case 2: return context.report(declaration, key + ': ' + propType + ' is a misspelled prop type. Please correct spelling');
        default: return context.report(declaration, key + ': ' + propType + ' is not a known prop type');
      }
    } else if (declaration.type === 'FunctionExpression') {
      // ok
    } else {
      context.report(declaration, key + ': unknown use of PropTypes');
    }
  }

  function checkProperties(declarations) {
    declarations.forEach((declaration) => {
      assertExpectedReactPropSyntax(isIdentifierOrRawLiteral(declaration), declaration.value, true);
    });
  }
