...
module.exports = function (context) {
  ...
  function assertExpectedReactPropSyntax(key, declaration, allowIsRequired) {
    if (declaration.type === 'MemberExpression' &&
      ...
      if (!allowIsRequired) {
        return context.report(declaration, "property '" + key + "': please move isRequired qualifier outside the oneOfType() declaration e.g. '(React.)PropTypes.oneOfType(...).isRequired'");
      }
      declaration = declaration.object;
    }
    if (declaration.type === 'CallExpression') {
      if (!isPropTypesExpression(declaration.callee) || !isIdentifier(declaration.callee.property)) {
        return context.report(declaration, "property '" + key + "': unsupported proptypes call syntax");
      }
      var propType = declaration.callee.property.name;
      var propValue = getPropValue(propType);
      switch (propValue) {
        case 1: return context.report(declaration, "property '" + key + "': " + propType + ' is a simple prop type, should not be called like a function');
        case 2: return context.report(declaration, "property '" + key + "': " + propType + ' is a misspelled prop type. Please correct spelling');
        case 3: case 4: case 5: case 6: case 7: break;
        default: return context.report(declaration, "property '" + key + "': " + propType + ' is not a known prop type');
      }
      if (declaration.arguments.length !== 1) {
        return context.report(declaration, "property '" + key + "': " + propType + ' expects exactly one argument of type ' + argType[propValue]);
      }
      ...
      switch (propValue) {
        ...
        case 4:
          if (arg.type === 'Literal') {
            context.report(declaration, "property '" + key + "': " + propType + ' argument must not be a literal');
          }
          break;
        case 5:
          arg.elements.every(function (elem) {
            if (elem.type !== 'Literal') {
              context.report(declaration, "property '" + key + "': " + propType + ' array entries must be literals');
              return false;
            }
            return true;
          });
        ...
        case 7:
          arg.properties.every(function (elem) {
            if (elem.type !== 'Property') {
              context.report(declaration, "property '" + key + "': " + propType + ' object must only consist of properties');
              return false;
            }
      ...
      switch (propValue) {
        case 1: return;
        case 2: return context.report(declaration, "property '" + key + "': " + propType + ' is a misspelled prop type. Please correct spelling');
        case 3: case 4: case 5: case 6: case 7: return context.report(declaration, "property '" + key + "': " + propType + ' expects to be called like a function with an argument of type ' + argType[propValue]);
        default: return context.report(declaration, "property '" + key + "': " + propType + ' is not a known prop type');
      }
